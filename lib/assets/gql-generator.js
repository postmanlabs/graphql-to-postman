...
var gqlSchema,
  ...
  // TODO: Create Variables object for each query generated.
  /**
...
module.exports = {
   ...
   *
   * @param {String} schema - String of GraphQL Schema in SDL format
   ...
   */
  schemaToQuery (schema, depthLimit = 1, includeDeprecatedFields = false) {
    const source = new graphql.Source(schema),
      output = {};

    gqlSchema = graphql.buildSchema(source);
      ...
      Object.keys(obj).forEach((type) => {
        ...
        if (includeDeprecatedFields || !field.isDeprecated) {
          ...
          Object.entries(queryResult.argumentsDict).map(([varName, arg]) => {
            variable[varName] = resolveVariableType(arg.type);
          });
