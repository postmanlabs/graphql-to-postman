    ...
    function generateQueries (obj, description) {
      ...
      switch (description) {
        case 'Mutation':
          ...
          currentObject.description = 'Mutations';
          break;
        case 'Query':
          outputFolderName = 'queries';
          currentObject.description = 'Queries';
          break;
        case 'Subscription':
          outputFolderName = 'subscriptions';
          currentObject.description = 'Subscriptions';
          break;
        default:
      ...
      Object.keys(obj).forEach((type) => {
        ...
        if (includeDeprecatedFields || !field.isDeprecated) {
          ...
          Object.entries(queryResult.argumentsDict).map(([varName, arg]) => {
            variables[varName] = resolveVariableType(arg.type, gqlSchema);
          });

          let query = queryResult.queryStr;
          query = `${description.toLowerCase()} ${type}${varsToTypesStr ? ` (${varsToTypesStr}) ` : ' '}{\n${query}\n}`;
          currentObject.items[type] = {
            query: query,
