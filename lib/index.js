...
function overrideOptions (options) {
  ...
  // Override depth to MAX_ALLOWED_DEPTH if more
  if (!options.__CLI && options.depth > MAX_ALLOWED_DEPTH) {
    options.depth = MAX_ALLOWED_DEPTH;
  }
  ...
  getOptions: function () {
    return [
      ...
      },
      {
        ...
        ' Maximum depth supported is 4, to work with more depth checkout' +
        ' [CLI usage](https://github.com/postmanlabs/graphql-to-postman#-command-line-interface).',
        external: true
      },
      {
      ...
      },
      {
        ...
        type: 'boolean',
        default: false,
        description: 'Indicates if current environment running env is CLI or not.',
        external: false
      }
    ];
  },
  ...
  convert: function (input, options, callback) {
    ...
    try {
      if (gqlSchemaObj) {
        ...
        // default variable depth should be 4.
      ...
      _.forEach(queryCollection, (value, key) => {
        ...
        folder.name = key;
        folder.description = value.description; // Just a simple string
        _.forEach(value.items, (graphqlObj, name) => {
          var request = {},
            item = {};
          ...
          request.url = '{{url}}';
          request.method = 'POST';
          if (graphqlObj.description) {
            request.description = {
              content: graphqlObj.description,
              type: 'text/markdown' // Since graphql descriptions support CommonMark
            };
            item.description = request.description;
          }
          delete graphqlObj.description;
          request.body = {
            mode: 'graphql',
            graphql: graphqlObj
          };
